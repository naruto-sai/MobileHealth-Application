 ...
 * The code if from google android samples
 */
    ...
    private final ConnectionLifecycleCallback mConnectionLifecycleCallback =
            new ConnectionLifecycleCallback() {
                ...
                public void onConnectionInitiated(String endpointId, ConnectionInfo connectionInfo) {
                    ...
                    NearbyConnectionsActivity.this.onConnectionInitiated(endpoint, connectionInfo);
                }
                ...
                public void onConnectionResult(String endpointId, ConnectionResolution result) {
                    ...
                    switch (result.getStatus().getStatusCode()) {
                        case ConnectionsStatusCodes.STATUS_OK:
                            logD(
                                    String.format(
                                            "Connection failed. Received status OK.",
                                            NearbyConnectionsActivity.toString(result.getStatus())));
                            connectedToEndpoint(mPendingConnections.remove(endpointId));
                            break;
                        case ConnectionsStatusCodes.STATUS_CONNECTION_REJECTED:
                            logW(
                                    String.format(
                                            "Connection failed. Received status REJECTED.",
                                            NearbyConnectionsActivity.toString(result.getStatus())));
                        ...
                        case ConnectionsStatusCodes.STATUS_ERROR:
                            logW(
                                    String.format(
                                            "Connection failed. Received status ERROR.",
                                            NearbyConnectionsActivity.toString(result.getStatus())));
                            onConnectionFailed(mPendingConnections.remove(endpointId));
                            break;
                    }
...
//                    connectedToEndpoint(mPendingConnections.remove(endpointId));
        ...
        if (requestCode == REQUEST_CODE_REQUIRED_PERMISSIONS) {
            for (int grantResult : grantResults) {
                if (grantResult == PackageManager.PERMISSION_DENIED) {
                    Toast.makeText(this, "Cannot start without required permissions",
    ...
    /**
     * An optional hook to pool any permissions the app needs with the permissions NearbyConnectionsActivity
     * will request.
     *
